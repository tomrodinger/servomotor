#!/bin/bash
# _v4shot.sh <query> <out.png> [w] [h] [cropbox]
set -u
Q="$1"; OUT="$2"; W="${3:-1440}"; H="${4:-820}"; CROP="${5:-}"
HERE="$(cd "$(dirname "$0")" && pwd)"
RAW="${OUT%.png}_full.png"
"$HERE/_shot.sh" "http://127.0.0.1:8912/premium_minimal_variations/v4.html?$Q" "$RAW" "$W" "$H" >/dev/null 2>&1
if [ -n "$CROP" ]; then
