#!/bin/bash
# TEMP verification helper for v9 slogan cycler (delete when done).
# Usage: ./_h9.sh "<query string without ?>" <out.png> <layoutWidth> <sliceTop> <sliceHeight>
set -u
QS="$1"; OUT="$2"; W="$3"; TOP="$4"; SH="$5"
HERE="$(cd "$(dirname "$0")" && pwd)"
HOSTW=$W; [ "$W" -lt 500 ] && HOSTW=500
ESC=$(printf '%s' "$QS" | sed 's/&/\&amp;/g')
H="$HERE/_h9frame_$$.html"
cat > "$H" <<EOF2
...
<div id="c"><iframe src="v9.html?${ESC}" scrolling="no"></iframe></div>
EOF2
