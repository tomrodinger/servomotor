#!/bin/bash
# TEMP: shoot a t-sweep for v5 pair i and tile it vertically. Delete when done.
# Usage: ./_strip5.sh <i> <out.png> [w] [top] [sh] [tlist]
set -u
I="$1"; OUT="$2"; W="${3:-1440}"; TOP="${4:-150}"; SH="${5:-330}"
TL="${6:-0.15,0.35,0.5,0.65,0.85}"
HERE="$(cd "$(dirname "$0")" && pwd)"
FILES=""
IFS=','; for t in $TL; do
  f="/tmp/_s5_${I}_${t}.png"
  "$HERE/_h5.sh" "motion=1&i=$I&t=$t" "$f" "$W" "$TOP" "$SH" >/dev/null 2>&1
  FILES="$FILES $f:$t"
done
unset IFS
