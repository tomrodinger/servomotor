...
void clear_the_queue_and_stop_no_disable_interrupt(void)
{
    ...
    velocity_after_last_queue_item = 0; // the planner's end-of-queue velocity must match the (now zero) runtime velocity, otherwise later moves are validated against a stale baseline
...
static void check_queue_item_prediction_overflow(int64_t acceleration, uint32_t n_time_steps)
{
    // Bound both prediction terms computed below so the int64 math cannot wrap
    // past the validation checks:
    //   velocity term: acceleration * n_time_steps
    //   position term: acceleration * (n_time_steps * (n_time_steps + 1) / 2)
    if(acceleration == 0) {
        return;
    }
    uint64_t limit = (uint64_t)((INT64_MAX >> 1) / llabs(acceleration));
    if((uint64_t)n_time_steps > limit) {
        fatal_error(ERROR_PREDICTED_VELOCITY_TOO_HIGH); // the velocity prediction would overflow int64 and wrap past this check
    }
    uint64_t triangular = ((uint64_t)n_time_steps * ((uint64_t)n_time_steps + 1)) >> 1;
    if(triangular > limit) {
        fatal_error(ERROR_PREDICTED_POSITION_OUT_OF_SAFETY_ZONE); // the position prediction would overflow int64 and wrap past the safety-zone checks
    }
}
...
//  char buf[150];
    ...
    if(n_time_steps == 0) {
        // Deliberately a silent no-op: internal callers rely on it (the trapezoid
        // planner emits a zero-duration coast segment for triangular profiles) and
        // zero-duration items inside a Multimove are documented to be dropped.
        // The user-facing single-move commands reject zero durations before
        // reaching this point (fatal error 34).
        return;
    }
    if(n_items_in_queue < MOVEMENT_QUEUE_SIZE) {
        ...
        if(movement_type == MOVE_WITH_ACCELERATION) {
            ...
            check_queue_item_prediction_overflow(movement_queue[queue_write_position].acceleration, n_time_steps);
...
//  int64_t predicted_final_position;
    ...
    if(n_time_steps == 0) {
        fatal_error(ERROR_PARAMETER_OUT_OF_RANGE); // match the real move commands, which reject zero-duration moves
    }
    if(movement_type == MOVE_WITH_ACCELERATION) {
        ...
        check_queue_item_prediction_overflow(movement_queue_queue_write_position_acceleration, n_time_steps);
...
//      sprintf(buf, "Predicted final position: " _PRId32 "\n", (int32_t)(predicted_final_position >> 32));
//      print_debug_string(buf);
        int64_t position_change_due_to_queue_item = velocity_after_last_queue_item * n_time_steps + movement_queue_queue_write_position_acceleration * (((uint64_t)n_time_steps * (n_time_steps + 1)) >> 1);
        add_int96(&position_after_last_queue_item_i96, &position_change_due_to_queue_item, &predicted_final_position_i96);
        int64_t predicted_final_position_i64 = predicted_final_position_i96.position;
//      if((((int32_t*)&predicted_final_position)[1] < position_lower_safety_limit) || (((int32_t*)&predicted_final_position)[1] > position_upper_safety_limit)) {
        if((predicted_final_position_i64 < position_lower_safety_limit_i64) || (predicted_final_position_i64 > position_upper_safety_limit_i64)) {
...
//      print_debug_string(buf);
    ...
    // This is a dry-run validator: report the predictions but do NOT commit
    // position_after_last_queue_item_i96 / velocity_after_last_queue_item,
    // otherwise later real moves are validated from a fictitious baseline.
...
void add_trapezoid_move_to_queue(int32_t total_displacement, uint32_t total_time)
{
    ...
    if(total_time == 0) {
        fatal_error(ERROR_PARAMETER_OUT_OF_RANGE); // a move over zero time was previously a silent no-op and divides by zero in the planner
    }
...
uint8_t handle_queued_movements(void)
{
    if(!decelerate_to_stop_active) {
        if(n_items_in_queue > 0) {
            ...
            // (The dry-run ADD_TO_QUEUE_TEST no longer corrupts the planner baseline,
            // so this consistency check now runs unconditionally)
...
void motor_phase_calculations(void)
{
    ...
    static int32_t phase2_slope;
    static int32_t tmp32bit;
    static int32_t phase1_int;
    static int32_t phase2_int;
    ...
    phase1_int >>= 8;
    phase1_int *= (uint32_t)motor_pwm_voltage;
    phase1_int >>= 16;
    phase1_int += PWM_PERIOD_TIM1 >> 1;
    if ((phase1_int < 0) || (phase1_int > PWM_PERIOD_TIM1)) {
    ...
    phase2_int >>= 8;
    phase2_int *= (uint32_t)motor_pwm_voltage;
    phase2_int >>= 16;
    phase2_int += PWM_PERIOD_TIM1 >> 1;
...
{
    if(new_max_velocity == 0) {
        fatal_error(ERROR_PARAMETER_OUT_OF_RANGE); // a zero limit makes the planner silently turn every trapezoid/go-to-position into a zero-motion dwell (success reply, queue occupied, shaft never moves) -- reject it, mirroring the zero check in set_max_acceleration
    }
    __disable_irq();
    max_velocity = new_max_velocity;
    max_velocity <<= VELOCITY_SHIFT_LEFT;
    if(max_velocity > EXPERIMENTAL_MAX_VELOCITY) { // the boot default is the rated MAX_VELOCITY; users may set up to this higher experimental ceiling
        max_velocity = EXPERIMENTAL_MAX_VELOCITY;
    }
    __enable_irq();
}
...
void set_max_acceleration(uint32_t new_max_acceleration)
{
    if(new_max_acceleration == 0) {
        fatal_error(ERROR_PARAMETER_OUT_OF_RANGE); // trapezoid planning divides by the maximum acceleration
    }
    __disable_irq();
    max_acceleration = new_max_acceleration;
    max_acceleration <<= ACCELERATION_SHIFT_LEFT;
    if(max_acceleration > EXPERIMENTAL_MAX_ACCELERATION) { // the boot default is the rated MAX_ACCELERATION; users may set up to this higher experimental ceiling
        max_acceleration = EXPERIMENTAL_MAX_ACCELERATION;
    }
    __enable_irq();
}
