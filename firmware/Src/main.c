...
#define BUGFIX_FIRMWARE_VERSION 12
    ...
    case MOVE_WITH_ACCELERATION_COMMAND:
        {
            ...
            rs485_done_with_this_packet();
            if(acceleration_input.time_steps == 0) {
                fatal_error(ERROR_PARAMETER_OUT_OF_RANGE); // a zero-duration move was previously a silent no-op
            }
    ...
    case MOVE_WITH_VELOCITY_COMMAND:
        {
            ...
            rs485_done_with_this_packet();
            if(velocity_input.time_steps == 0) {
                fatal_error(ERROR_PARAMETER_OUT_OF_RANGE); // a zero-duration move was previously a silent no-op
            }
    ...
    case MULTIMOVE_COMMAND:
        {
            ...
            if(n_moves_in_this_command > MAX_MULTIMOVES) {
                fatal_error(ERROR_MULTIMOVE_MORE_THAN_32_MOVES); // must be checked BEFORE the copy below: a larger count would overflow the fixed-size buffer
            }
            typedef struct __attribute__((__packed__)) {
...
//            uint16_t expected_size = sizeof(n_moves_in_this_command) + sizeof(multi_move_command_buffer);
    ...
    //            print_debug_string(buf);
            if (limits_input.lower_limit > limits_input.upper_limit) {
                fatal_error(ERROR_PARAMETER_OUT_OF_RANGE); // inverted limits would put every position out of bounds and trip the per-tick safety check within one control tick, racing this command's own reply; reject them deterministically here instead (the host receives this code as the error reply)
            }
    ...
    case READ_MULTIPURPOSE_BUFFER_COMMAND:
        ...
        if(!is_broadcast) {
            ...
            if (data_type == 0) {
                // The buffer is empty: reply with just the data_type byte of 0 so
                // the host gets a definite answer instead of a timeout
                struct __attribute__((__packed__)) {
                    uint8_t header[3]; // filled in by rs485_finalize_and_transmit_packet()
                    uint8_t data_type;
                    uint32_t crc32; // filled in by rs485_finalize_and_transmit_packet()
                } empty_buffer_response;
                empty_buffer_response.data_type = 0;
                rs485_finalize_and_transmit_packet(&empty_buffer_response, sizeof(empty_buffer_response));
            }
            if (data_type != 0) {
                struct __attribute__((__packed__)) {
                    uint8_t packet_size;
    ...
    case TEST_MODE_COMMAND:
        {
            ...
            rs485_done_with_this_packet();
            if (test_mode == 0) {
                // Clear all clearable test modes and restore defaults. Note: this
                // must NOT call set_led_test_mode(), which never returns.
                set_motor_test_mode(0);
                set_overtemperature_test_mode(0);
                set_overvoltage_threshold(OVERVOLTAGE_PROTECTION_SETTING);
            }
    ...
    case SET_MAX_ALLOWABLE_POSITION_DEVIATION:
        {
            ...
            rs485_done_with_this_packet();
            if (new_max_allowable_position_deviation == INT64_MIN) {
                new_max_allowable_position_deviation = INT64_MAX; // llabs(INT64_MIN) is undefined behavior and stays negative on ARM, which made the per-tick deviation check trip on every tick (BUG-24); saturate to complete the documented absolute-value contract
            }
