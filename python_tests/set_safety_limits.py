...
def send_set_safety_limits_command(ser, alias, lower_limit, upper_limit):
    command = bytearray([alias, SET_SAFETY_LIMITS_COMMAND, 8])
    command = command + lower_limit.to_bytes(4, byteorder = "little", signed = True)
    command = command + upper_limit.to_bytes(4, "little", signed = True)
    print("Writing %d bytes" % (len(command)))
    print_data(command)
    ser.write(command)
...
parser = argparse.ArgumentParser(description='Set the safety limits. If the motor tries to move past them, an error will be thrown')
parser.add_argument('-p', '--port', help='serial port device', default=None)
parser.add_argument('-a', '--alias', help='alias of the device to control', default=None)
parser.add_argument('lower_limit', help='This is the lower safety limit in mm')
parser.add_argument('upper_limit', help='This is the upper safety limit in mm')
...
lower_limit_mm = float(args.lower_limit)
upper_limit_mm = float(args.upper_limit)
...
lower_limit_microsteps = int(lower_limit_mm * microsteps_per_mm)
upper_limit_microsteps = int(upper_limit_mm * microsteps_per_mm)

ser = serial_functions.open_serial_port(serial_port, 230400, 0.05)

send_set_safety_limits_command(ser, alias, lower_limit_microsteps, upper_limit_microsteps)
