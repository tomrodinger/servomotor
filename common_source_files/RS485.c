...
void USART1_IRQHandler(void)
{
    ...
    if((USART1->ISR & USART_ISR_RXNE_RXFNE) && (USART1->CR1 & USART_CR1_RXNEIE_RXFNEIE)) {
        ...
        if(nReceivedBytes == 1) {
            // Validate that the Device ID byte has LSB set to 1 (except for special values)
            if(!is_valid_device_id_format(receivedByte)) {
                // Invalid device ID format - LSB not set to 1
            ...
            if(receiveIndex >= valueLength) {
                if(selectedAxis != RESPONSE_CHARACTER && (selectedAxis == global_settings.my_alias || selectedAxis == ALL_ALIAS)) {
