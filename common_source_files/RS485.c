...
void rs485_init(void)
{
    ...
    USART1->RTOR = ((230400 / 10) << USART_RTOR_RTO_Pos); // set the timeout to 0.1 s)
...
void USART1_IRQHandler(void)
{
    ...
    if(USART1->ISR & USART_ISR_FE) {
        framing_error_count++;
        USART1->ICR |= USART_ICR_FECF; // clear this framing error
//        fatal_error(ERROR_FRAMING); // All error messages are defined in error_text.h, which is an autogenerated file based on error_codes.json in the servomotor Python module (<repo root>/python_programs/servomotor/error_codes.json)
    ...
    if(USART1->ISR & USART_ISR_ORE) {
        overrun_error_count++;
        USART1->ICR |= USART_ICR_ORECF; // clear this overrun error
//        fatal_error(ERROR_OVERRUN); // All error messages are defined in error_text.h, which is an autogenerated file based on error_codes.json in the servomotor Python module (<repo root>/python_programs/servomotor/error_codes.json)
    ...
    if(USART1->ISR & USART_ISR_NE) {
        noise_error_count++;
        USART1->ICR |= USART_ICR_NECF; // clear this noise error
...
void rs485_transmit_error_packet(uint8_t error_code)
{
    if (crc32_enabled) {
        ...
        response_with_crc32.size = encode_first_byte(sizeof(response_with_crc32));
        response_with_crc32.response_character = RESPONSE_CHARACTER_CRC32_ENABLED;
        response_with_crc32.error_code = error_code;
        ...
        struct __attribute__((__packed__)) {
            ...
            uint8_t response_character;
            uint8_t error_code;
        } response_without_crc32;

        response_without_crc32.size = encode_first_byte(sizeof(response_without_crc32));
        response_without_crc32.response_character = RESPONSE_CHARACTER_CRC32_DISABLED;
        response_without_crc32.error_code = error_code;
        rs485_transmit(&response_without_crc32, sizeof(response_without_crc32));
    }
}
...
void rs485_finalize_and_transmit_packet(void *data, uint16_t structure_size)
{
    ...
    ((uint8_t*)data)[2] = ERROR_CODE_NO_ERROR; // indicates that there is no fatal error at this time
...
void rs485_start_the_packet(uint16_t payload_size)
{
    ...
    header.error_code = ERROR_CODE_NO_ERROR;
    if(crc32_enabled) {
