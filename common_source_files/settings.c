...
static void unlock_flash(void)
{
    ...
    if(FLASH->CR & FLASH_CR_LOCK_Msk) {
        fatal_error(ERROR_FLASH_UNLOCK_FAIL); // All error messages are defined in error_text.h, which is an autogenerated file based on error_codes.json in the servomotor Python module (<repo root>/python_programs/servomotor/error_codes.json)
    }
}
...
//        while(FLASH->SR & FLASH_SR_BSY1_Msk);
        ...
        *(dst_memory_location++) = data_u32;
        ...
        if((FLASH->SR & FLASH_SR_EOP_Msk) == 0) {
            fatal_error(ERROR_FLASH_WRITE_FAIL); // All error messages are defined in error_text.h, which is an autogenerated file based on error_codes.json in the servomotor Python module (<repo root>/python_programs/servomotor/error_codes.json)
        }
