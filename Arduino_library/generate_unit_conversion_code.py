...
def main(json_filename):
    ...
    with open(CPP_FILENAME, 'w') as cf:
        ...
        for category, unit_list in units_dict.items():
            ...
            func_name = f"convert{category.capitalize()}"
            ref_unit = unit_list[0]  # First unit in the list is the reference unit
            ...
            for u in unit_list:
                ...
                # When converting from a unit to reference, multiply by factor/reference
                factor_from = factor / ref_factor if ref_factor != 0 else 0.0
            ...
            for u in unit_list:
                enumerator = u.upper().replace(" ", "_")
                factor = factors.get(u, 1.0)
                # When converting from reference to a unit, multiply by reference/factor
                factor_to = ref_factor / factor if factor != 0 else 0.0
                cf.write(f"      case {enum_name}::{enumerator}:\n")
                cf.write(f"        {{\n")
