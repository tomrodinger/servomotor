...
extern int8_t M_index;
extern int32_t p_value, i_value, d_value;

static uint64_t my_unique_id;
static int16_t detect_devices_delay = -1;

static uint8_t heating_flag = 0;
uint16_t target_temp_extruder = 0;
uint16_t sensor_val_extruder = 0;
uint32_t heating_counter = 0;
...
void portA_init(void)
{
    ...
    GPIOA->MODER =
            ...
            (MODER_DIGITAL_INPUT      << GPIO_MODER_MODE13_Pos) | // Button input and also SWDIO (for programming)
            (MODER_ANALOG_INPUT       << GPIO_MODER_MODE14_Pos) | // SWCLK (for programming)
            (MODER_DIGITAL_INPUT      << GPIO_MODER_MODE15_Pos);  // touch
...
void portB_init(void)
{
    #define HEATER_PORT_B_PIN 2

    GPIOB->MODER =
            (MODER_ALTERNATE_FUNCTION << GPIO_MODER_MODE0_Pos)  | //temperature sensor(ADC_IN8) | PWM 3B(TIM3 channel 3)
            (MODER_DIGITAL_OUTPUT     << GPIO_MODER_MODE1_Pos)  | //heater | IB sensor(TIM3 channel 4)
            (MODER_ANALOG_INPUT       << GPIO_MODER_MODE2_Pos)  | //temperature(new)(ADC_IN10)
            ...
            (MODER_ALTERNATE_FUNCTION << GPIO_MODER_MODE6_Pos)  | // RS485 Data out
            (MODER_ALTERNATE_FUNCTION << GPIO_MODER_MODE7_Pos)  | // RS485 Data receive
            (MODER_DIGITAL_OUTPUT     << GPIO_MODER_MODE8_Pos)  |
    ...
    GPIOB->OTYPER = (OTYPER_OPEN_DRAIN << GPIO_OTYPER_OT7_Pos);
                     // step input make as open drain
    	             // direction input make as open drain
    		         // RX pin make as open drain
    GPIOB->OSPEEDR = 0xffffffff; // make all pins very high speed
    GPIOB->PUPDR = (PUPDR_PULL_UP << GPIO_PUPDR_PUPD7_Pos) | (PUPDR_PULL_DOWN << GPIO_PUPDR_PUPD12_Pos); // RX pin is pull up, overvoltage pin is pull down
}
...
//            transmit(buf, strlen(buf));
        ...
        case SET_EXTRUDER_TEMPERATURE_COMMAND:
            target_temp_extruder = *(uint16_t*)parameters;
            rs485_allow_next_command();
            if(axis != ALL_ALIAS) {
            	if(target_temp_extruder > 0) {
            		heating_flag = 1;
            		rs485_transmit(NO_ERROR_RESPONSE, 3);
            	}
            	else {
        ...
        case SET_PID_D_VALUE_COMMAND:
            d_value = *(int32_t*)parameters;
            rs485_allow_next_command();
            if(axis != ALL_ALIAS) {
                rs485_transmit(NO_ERROR_RESPONSE, 3);
            }
            break;
        case GET_PID_VALUE_COMMAND:
            rs485_allow_next_command();
            if(axis != ALL_ALIAS) {
                rs485_transmit("R\x01\x0c", 3);
                rs485_transmit(&p_value, 4);
                rs485_transmit(&i_value, 4);
                rs485_transmit(&d_value, 4);
            }
            break;
        }
    }
...
{
    sensor_val_extruder = get_temperature_sensor_voltage();
    heating_counter = (heating_counter + 1) & 1023;
    if(heating_counter % 1023 != 0) return;
    // print_hall_position();
    ...
    if(heating_flag) {
        if(sensor_val_extruder > target_temp_extruder) {
            GPIOA->ODR |= (1 << HEATER_PORT_A_PIN);
            // GPIOB->ODR |= (1 << HEATER_PORT_B_PIN);
        }
        else {
