...
#define ENABLE_MOTOR_UPDATES() TIM1->DIER |= TIM_DIER_UIE    // enable the update interrupt
#define DISABLE_MOTOR_UPDATES() TIM1->DIER &= ~TIM_DIER_UIE  // disable the update interrupt during this operation
//#define ENABLE_MOTOR_UPDATES() TIM3->DIER |= TIM_DIER_UIE    // enable the update interrupt
//#define DISABLE_MOTOR_UPDATES() TIM3->DIER &= ~TIM_DIER_UIE  // disable the update interrupt during this operation
...
void start_calibration(uint8_t print_output)
{
	...
	if((M_index != 0) && (M_index != 1)) {
		fatal_error(33); // "Invalid run mode" (all error text is defined in error_text.c)
	}
...
{
    DISABLE_MOTOR_UPDATES(); // disable the update interrupt
}
