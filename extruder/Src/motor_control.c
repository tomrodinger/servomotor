...
#define MAX_PWM_VOLTAGE (100)
#define OPEN_LOOP_STATIC_MOTOR_PWM_VOLTAGE (100)
// #define ANALOG_WATCHDOG_LIMIT_MULTIPLIER (320)
#define ANALOG_WATCHDOG_LIMIT_MULTIPLIER (1024)
#define VOLTS_PER_ROTATIONAL_VELOCITY 300ss
...
// #define EXPECTED_MOTOR_CURRENT_BASELINE 1689
#define MOTOR_CURRENT_BASELINE_TOLERANCE 2000
...
static uint32_t calibration_index[2];
...
typedef struct {
	...
	// struct fast_capture_data_struct *fast_capture_data;
	struct capture_struct capture;
	struct homing_struct homing;
	struct closed_loop_struct closed_loop;

	// uint16_t fast_capture_data_size;
	// uint16_t fast_capture_data_index;
	// uint8_t fast_capture_data_active;
	// uint8_t fast_capture_data_result_ready;
	// uint32_t calibration_index[3];

	int32_t hall_position;
	int32_t previous_hall_position;
	int32_t hall_position_delta;
	int32_t velocity;
	int64_t current_velocity_i64;
	int64_t current_position_i64;
	uint32_t commutation_position;
	uint32_t commutation_position_offset;
	int64_t max_acceleration;
	int64_t max_velocity;
	int32_t motor_pwm_voltage;
	int32_t motor_pwm_voltage_with_sign;
  int32_t max_pwm_voltage;
	int32_t desired_motor_pwm_voltage;
	uint8_t motor_control_mode;

	uint8_t go_to_closed_loop_mode_active;
	int32_t commutation_scan_microsteps;
	uint8_t vibration_four_step;
	int32_t sum_delta;
	int32_t motor_pwm_voltage_limit;
	int32_t vibration_polarity_moving_average;
	int32_t max_motor_pwm_voltage_limit;
	uint32_t commutation_position_offset_at_max;
	uint16_t motor_current_baseline;
	int32_t position_lower_safety_limit;
	int32_t position_upper_safety_limit;

	uint8_t homing_active;
	uint8_t calibration_active;
	uint8_t calibration_print_output;
	uint8_t decelerate_to_stop_active;
	uint8_t motor_busy; // this will be set to 1 while the motor is doing a long action like calibration or homing. other movement commands cannot be executed during this time.

} motor_struct_t;

static motor_struct_t motor_list[NUMBER_OF_MOTORS];
int32_t p_value = 600;
int32_t i_value = 2;
int32_t d_value = 60;
...
void motor_struct_init() {
	for(int i = 0; i < NUMBER_OF_MOTORS; i++) {
		// motor_list[i].fast_capture_data = (void*)&(motor_list[i].calibration);
	  // motor_list[i].fast_capture_data_size = sizeof(motor_list[i].calibration) / sizeof(struct fast_capture_data_struct);
		// motor_list[i].fast_capture_data_active = 0;
		// motor_list[i].fast_capture_data_result_ready = 0;
		motor_list[i].capture = (struct capture_struct){0};
		motor_list[i].homing = (struct homing_struct){0};
		motor_list[i].closed_loop = (struct closed_loop_struct){0};
	}
	for(int i = 0; i < NUMBER_OF_MOTORS; i++) {
		motor_list[i].hall_position = 0;
		motor_list[i].velocity = 0;
		motor_list[i].current_velocity_i64 = 0;
		motor_list[i].current_position_i64 = 0;
		motor_list[i].commutation_position = 0;
		motor_list[i].commutation_position_offset = UINT32_MIDPOINT;
		motor_list[i].max_acceleration = MAX_ACCELERATION;
		motor_list[i].max_velocity = MAX_VELOCITY;
		motor_list[i].motor_pwm_voltage = 0;
		motor_list[i].motor_pwm_voltage_with_sign = 0;
		motor_list[i].max_pwm_voltage = 0;
		motor_list[i].desired_motor_pwm_voltage = 0;
		motor_list[i].motor_control_mode = OPEN_LOOP_POSITION_CONTROL;

		motor_list[i].go_to_closed_loop_mode_active = 0;
		motor_list[i].motor_current_baseline = 1350;
		motor_list[i].position_lower_safety_limit = -2000000000;
		motor_list[i].position_upper_safety_limit = 2000000000;

		motor_list[i].homing_active = 0;
		motor_list[i].calibration_active = 0;
		motor_list[i].decelerate_to_stop_active = 0;
		motor_list[i].motor_busy = 0;
	}
}
...
//	uint32_t delta_t1 = ((max_acceleration >> 1) + max_velocity) / max_acceleration; // calculating detal_t1 with rounding
	int64_t delta_t1 = motor_list[M_index].max_velocity / motor_list[M_index].max_acceleration; // calculating detal_t1 without rounding
	...
	print_int64("max_velocity: ", motor_list[M_index].max_velocity);
	print_int64("max_acceleration: ", motor_list[M_index].max_acceleration);
...
uint8_t hall_rising_flag[2];
uint16_t hall_local_maximum[2];
uint16_t hall_local_minimum[2];
int32_t hall_calibration_start_position;
int32_t hall_local_maximum_position[2];
int32_t hall_local_minimum_position[2];
...
void start_calibration(uint8_t print_output)
{
	...
	if(motor_list[M_index].motor_control_mode != OPEN_LOOP_POSITION_CONTROL) {
	...
	motor_list[M_index].calibration_print_output = print_output;
	...
	motor_list[M_index].motor_busy = 1;
...
/*
	...
	add_trapezoid_move_to_queue(-CALIBRATION_DISTANCE, CALIBRATION_TIME / 2);
*/
	hall_calibration_start_position = ((int32_t *)&motor_list[M_index].current_position_i64)[1];
	for(j = 0; j < 2; j++) {
	...
	motor_list[M_index].calibration_active = 1;
}
...
void handle_calibration_logic(void)
{
	...
	int32_t calibration_relative_position = ((int32_t *)&motor_list[M_index].current_position_i64)[1] - hall_calibration_start_position;
	uint8_t j;

	if(n_items_in_queue == 2) {
		if(motor_list[M_index].calibration_print_output) {
			if(M_index == 0) {
				hall1_sum += get_smoothed_hall_sensor1_voltage();
				hall2_sum += get_smoothed_hall_sensor2_voltage();
			} else {
				hall1_sum += get_smoothed_hall_sensor1_voltage();
				hall2_sum += get_smoothed_hall_sensor2_voltage();
			}
			...
			if(avg_counter == 16) {
				...
				rs485_transmit((void*)hall_data_buffer, 4);
				avg_counter = 0;
				hall1_sum = 0;
				hall2_sum = 0;
			}
			...
			for(j = 0; j < 2; j++) {
				if(calibration_index[j] < CALIBRATION_DATA_SIZE) {
					...
					case 0:
					  if(M_index == 0) {
							hall_reading = get_smoothed_hall_sensor1_voltage();
						} else {
							hall_reading = get_smoothed_hall_sensor3_voltage();
						}
						break;
					case 1:
					  if(M_index == 0) {
							hall_reading = get_smoothed_hall_sensor2_voltage();
						} else {
							hall_reading = get_smoothed_hall_sensor4_voltage();
						}
						break;
					}
		...
		motor_list[M_index].calibration_active = 0;
		disable_mosfets();
		if(motor_list[M_index].calibration_print_output) {
	      rs485_transmit("Calibration capture done\n", 25);
	   		motor_list[M_index].motor_busy = 0;
		}
		else {
...
void process_calibration_data(void)
{
	...
	for(j = 0; j < 2; j++) {
		if(calibration_index[j] < MIN_CALIBRATION_LOCAL_MINIMA_OR_MAXIMA) {
			// sprintf(buf, "found %ld local_min_or_max from hall sensor %d\n", calibration_index[j], j + 1);
			// transmit(buf, strlen(buf));
		...
		midline[j] = minima_and_maxima_avg[j];
	}
...
#define GO_TO_CLOSED_LOOP_VIBRATION_MAGNITUDE 200
...
void start_go_to_closed_loop_mode(void)
{
	...
	enable_mosfets();
	motor_list[M_index].commutation_position_offset = UINT32_MIDPOINT;
	motor_list[M_index].vibration_four_step = 0;
	motor_list[M_index].commutation_scan_microsteps = 0;
	motor_list[M_index].desired_motor_pwm_voltage = 0;
	motor_list[M_index].go_to_closed_loop_mode_active = 1;
	motor_list[M_index].motor_pwm_voltage_limit = MOTOR_PWM_VOLTAGE_LIMIT_MINIMUM;
	motor_list[M_index].sum_delta = 0;
	motor_list[M_index].vibration_polarity_moving_average = 0;
	motor_list[M_index].max_motor_pwm_voltage_limit = 0;
	zero_hall_position();
	motor_list[M_index].previous_hall_position = 0;
...
{
	if(motor_list[M_index].commutation_scan_microsteps < COMMUTATION_SCAN_MICROSTEPS) {
		switch(motor_list[M_index].vibration_four_step) {
		case 0:
			motor_list[M_index].sum_delta += motor_list[M_index].hall_position_delta;
			motor_list[M_index].desired_motor_pwm_voltage++;
			if(motor_list[M_index].desired_motor_pwm_voltage >= motor_list[M_index].motor_pwm_voltage_limit) {
				motor_list[M_index].vibration_four_step++;
			}
			break;
		case 1:
		case 2:
		...
		case 3:
			motor_list[M_index].sum_delta += motor_list[M_index].hall_position_delta;
			motor_list[M_index].desired_motor_pwm_voltage++;
			if(motor_list[M_index].desired_motor_pwm_voltage >= 0) {
				motor_list[M_index].vibration_four_step = 0;
				// int32_t step_size = (MOTOR_PWM_VOLTAGE_LIMIT_MAXIMUM - motor_list[M_index].motor_pwm_voltage_limit + 1) * COMMUTATION_SCAN_STEP_FACTOR;
				// if(step_size < COMMUTATION_SCAN_MIN_STEP_SIZE) {
				// 	step_size = COMMUTATION_SCAN_MIN_STEP_SIZE;
				// }
				int32_t step_size = 100;
				motor_list[M_index].commutation_scan_microsteps += step_size;
				if(motor_list[M_index].sum_delta > 0) {
					if(motor_list[M_index].vibration_polarity_moving_average >= 0) {
						motor_list[M_index].vibration_polarity_moving_average++;
					}
					else {
						motor_list[M_index].vibration_polarity_moving_average += 2;
					}
				}
				else if(motor_list[M_index].sum_delta < 0) {
					if(motor_list[M_index].vibration_polarity_moving_average <= 0) {
						motor_list[M_index].vibration_polarity_moving_average--;
					}
					else {
...
//				vibration_polarity_moving_average = (vibration_polarity_moving_average * ((1 << MOVING_AVERAGE_SHIFT_RIGHT) - 1) + sum_delta) >> MOVING_AVERAGE_SHIFT_RIGHT;
				if(motor_list[M_index].sum_delta >= 0) {
					// print_number("%d", (uint16_t)motor_list[M_index].sum_delta);
				}
				else {
					motor_list[M_index].sum_delta = -motor_list[M_index].sum_delta;
					// print_number("-%d", (uint16_t)motor_list[M_index].sum_delta);
				}
				if(motor_list[M_index].sum_delta > GO_TO_CLOSED_LOOP_VIBRATION_MAGNITUDE) {
					if(motor_list[M_index].motor_pwm_voltage_limit > MOTOR_PWM_VOLTAGE_LIMIT_MINIMUM) {
						motor_list[M_index].motor_pwm_voltage_limit--;
					}
				}
				else {
					if(motor_list[M_index].motor_pwm_voltage_limit < MOTOR_PWM_VOLTAGE_LIMIT_MAXIMUM) {
						motor_list[M_index].motor_pwm_voltage_limit++;
					}
				}
				if(motor_list[M_index].motor_pwm_voltage_limit > motor_list[M_index].max_motor_pwm_voltage_limit) {
					motor_list[M_index].max_motor_pwm_voltage_limit = motor_list[M_index].motor_pwm_voltage_limit;
					motor_list[M_index].commutation_position_offset_at_max = motor_list[M_index].commutation_position_offset;
					if(motor_list[M_index].vibration_polarity_moving_average < 0) {
						motor_list[M_index].commutation_position_offset_at_max -= HALL_TO_POSITION_90_DEGREE_OFFSET;
					}
					else {
						motor_list[M_index].commutation_position_offset_at_max += HALL_TO_POSITION_90_DEGREE_OFFSET;
					}
				}
				motor_list[M_index].commutation_position_offset += step_size;
				// sprintf(buf, "%hu %ld\n", (uint16_t)motor_list[M_index].motor_pwm_voltage_limit, motor_list[M_index].vibration_polarity_moving_average);
				// transmit(buf, strlen(buf));
				motor_list[M_index].sum_delta = 0;
			}
			break;
		...
		motor_list[M_index].commutation_position_offset = motor_list[M_index].commutation_position_offset_at_max;
		motor_list[M_index].current_position_i64 = 0;
		motor_list[M_index].current_velocity_i64 = 0;
		motor_list[M_index].motor_pwm_voltage = 0;
		set_motor_control_mode(CLOSED_LOOP_POSITION_CONTROL);
		motor_list[M_index].hall_position = get_hall_position();
		motor_list[M_index].go_to_closed_loop_mode_active = 0;
		motor_list[M_index].motor_busy = 0;
    }
}
...
void start_capture(uint8_t capture_type)
{
    ...
    memset(&motor_list[M_index].capture, 0, sizeof(motor_list[M_index].capture));
    motor_list[M_index].capture.capture_type = capture_type;
...
{
    uint16_t hall_data_buffer[5];
    int32_t adjusted_hall_sensor_readings[2];
    static uint16_t counter = 0;

    if(motor_list[M_index].capture.capture_type == CAPTURE_HALL_SENSOR_READINGS) {
    	if(counter == 0) {
    		counter = 256;
				if(M_index == 0) {
	    		...
	    		hall_data_buffer[1] = (get_hall_sensor2_voltage() << 3) - HALL_SENSOR_SHIFT;
					...
					hall_data_buffer[0] = (get_hall_sensor3_voltage() << 3) - HALL_SENSOR_SHIFT;
	    		hall_data_buffer[1] = (get_hall_sensor4_voltage() << 3) - HALL_SENSOR_SHIFT;
				}
    ...
    else if(motor_list[M_index].capture.capture_type == CAPTURE_HALL_POSITION) {
    	if(counter == 0) {
    		counter = 256;
    ...
    else if(motor_list[M_index].capture.capture_type == CAPTURE_ADJUSTED_HALL_SENSOR_READINGS) {
    	if(counter == 0) {
    		counter = 256;
    		int32_t hall_position = get_hall_position();
...
{
	if(motor_list[M_index].motor_control_mode != CLOSED_LOOP_POSITION_CONTROL) {
	...
	motor_list[M_index].homing_active = 1;
}
...
void handle_homing_logic(void)
{
	...
	position_error = abs(((int32_t *)&motor_list[M_index].current_position_i64)[1] - motor_list[M_index].hall_position);

	if(position_error > HOMING_MAX_POSITION_ERROR) {
		motor_list[M_index].homing_active = 0;
		clear_the_queue_and_stop_no_disable_interrupt();
		motor_list[M_index].current_velocity_i64 = 0; // detected a colision so stop where we are
		if(((int32_t *)&motor_list[M_index].current_position_i64)[1] >= motor_list[M_index].hall_position) {
			((int32_t *)&motor_list[M_index].current_position_i64)[1] -= HOMING_MAX_POSITION_ERROR;
		}
		else {
			((int32_t *)&motor_list[M_index].current_position_i64)[1] += HOMING_MAX_POSITION_ERROR;
		}
...
{
	char buf[30];
	// sprintf(buf, "%ld\n", motor_list[M_index].hall_position);
	sprintf(buf, "%ld %ld\n", motor_list[M_index].hall_position, motor_list[M_index].motor_pwm_voltage);
	// sprintf(buf, "%ld %ld %ld\n", motor_list[M_index].hall_position, ((int32_t *)&motor_list[M_index].current_position_i64)[1], motor_list[M_index].motor_pwm_voltage);
	// sprintf(buf, "%ld %ld %ld\n", motor_list[M_index].hall_position, motor_list[M_index].commutation_position, motor_list[M_index].motor_pwm_voltage);
	transmit(buf, strlen(buf));
}
...
{
	char buf[100];
	sprintf(buf, "current_position: %ld\n", ((int32_t *)&motor_list[M_index].current_position_i64)[1]);
	transmit(buf, strlen(buf));
}
...
void print_current_movement(void)
{
	...
	print_int64("max acceleration:", (int64_t)motor_list[M_index].max_acceleration);
	print_int64("max velocity:", (int64_t)motor_list[M_index].max_velocity);
	sprintf(buf, "current_time: %lu\n", (unsigned long int)current_time);
	transmit(buf, strlen(buf));
	sprintf(buf, "motor_control_mode: %u\n", (unsigned int)motor_list[M_index].motor_control_mode);
	transmit(buf, strlen(buf));
}
...
void print_motor_current(void)
{
	...
	sprintf(buf, "current: %hd   motor_current_baseline: %hu\n", current, motor_list[M_index].motor_current_baseline);
	transmit(buf, strlen(buf));
}
...
void print_hall_sensor_data(void)
{
	...
	sprintf(buf, "hall_position: %ld   commutation_position_offset: %lu\n", motor_list[M_index].hall_position, motor_list[M_index].commutation_position_offset);
	transmit(buf, strlen(buf));
}
...
//	char buf[150];
	...
	if(motor_list[M_index].motor_busy) {
    ...
    if(n_items_in_queue < MOVEMENT_QUEUE_SIZE) {
		...
		if(movement_type == MOVE_WITH_ACCELERATION) {
            ...
            if(abs(movement_queue[queue_write_position].acceleration) > motor_list[M_index].max_acceleration) {
...
//			transmit(buf, strlen(buf));
			if(abs(predicted_final_velocity) > motor_list[M_index].max_velocity) {
...
//			transmit(buf, strlen(buf));
			if((((int32_t*)&predicted_final_position)[1] < motor_list[M_index].position_lower_safety_limit) || (((int32_t*)&predicted_final_position)[1] > motor_list[M_index].position_upper_safety_limit)) {
...
//					transmit(buf, strlen(buf));
					...
					if((((int32_t*)&absolute_position_at_turn_point)[1] < motor_list[M_index].position_lower_safety_limit) || (((int32_t*)&absolute_position_at_turn_point)[1] > motor_list[M_index].position_upper_safety_limit)) {
...
//			transmit(buf, strlen(buf));
            if(abs(movement_queue[queue_write_position].velocity) > motor_list[M_index].max_velocity) {
...
void add_to_queue_test(int32_t parameter, uint32_t n_time_steps, movement_type_t movement_type, add_to_queue_test_results_t *results)
{
	...
	if(movement_type == MOVE_WITH_ACCELERATION) {
		...
		if(abs(movement_queue_queue_write_position_acceleration) > motor_list[M_index].max_acceleration) {
			fatal_error(15); // "accel too high" (all error text is defined in error_text.c)
		}
		...
		sprintf(buf, "Predicted final velocity: %ld\n", ((int32_t*)&predicted_final_velocity)[1]);
		transmit(buf, strlen(buf));
		if(abs(movement_queue_queue_write_position_velocity) > motor_list[M_index].max_velocity) {
			fatal_error(16); // "vel too high" (all error text is defined in error_text.c)
		}
...
void compute_velocity(void)
{
	...
	velocity_moving_average = (VELOCITY_AVERAGING_TIME_STEPS * velocity_moving_average + (motor_list[M_index].hall_position_delta << 8) + (1 << (VELOCITY_AVERAGING_SHIFT_RIGHT - 1))) >> VELOCITY_AVERAGING_SHIFT_RIGHT;
	motor_list[M_index].velocity = ((velocity_moving_average + (1 << (8 - 1))) >> 8);
}

uint8_t handle_queued_movements(void)
{
	if(!motor_list[M_index].decelerate_to_stop_active) {
		if(n_items_in_queue > 0) {
			...
			if(movement_queue[queue_read_position].movement_type == MOVE_WITH_ACCELERATION) {
				motor_list[M_index].current_velocity_i64 += movement_queue[queue_read_position].acceleration; // consume one time step worth of acceleration
				...
				motor_list[M_index].current_velocity_i64 = movement_queue[queue_read_position].velocity; // velocity is constant during this time step
				movement_queue[queue_read_position].n_time_steps--;
				if(movement_queue[queue_read_position].n_time_steps == 0) {
					queue_read_position = (queue_read_position + 1) & (MOVEMENT_QUEUE_SIZE - 1);
					n_items_in_queue--;
			...
			motor_list[M_index].decelerate_to_stop_active = 1;
		}
	}
	if(motor_list[M_index].decelerate_to_stop_active) {
		if(motor_list[M_index].current_velocity_i64 != 0) {
			fatal_error(18); // "run out of queue items" (all error text is defined in error_text.c)
		}
		if(motor_list[M_index].current_velocity_i64 >= 0) {
			if(motor_list[M_index].current_velocity_i64 > motor_list[M_index].max_acceleration) {
				motor_list[M_index].current_velocity_i64 -= motor_list[M_index].max_acceleration;
			}
			else {
				motor_list[M_index].current_velocity_i64 = 0;
			}
		...
		if(motor_list[M_index].current_velocity_i64 == 0) {
			motor_list[M_index].decelerate_to_stop_active = 0;
		}
	}

	if((motor_list[M_index].current_velocity_i64 > motor_list[M_index].max_velocity) || (motor_list[M_index].current_velocity_i64 < -motor_list[M_index].max_velocity)) {
		fatal_error(16); // "vel too high" (all error text is defined in error_text.c)
	}
	motor_list[M_index].current_position_i64 += motor_list[M_index].current_velocity_i64;

	return (motor_list[M_index].current_velocity_i64 != 0);
...
#define PROPORTIONAL_CONSTANT_PID 800
#define INTEGRAL_CONSTANT_PID     1
#define DERIVATIVE_CONSTANT_PID   50
// #define PROPORTIONAL_CONSTANT_PID 1000
...
int32_t PID_controller(int32_t error)
{
    ...
    integral_term += (error * INTEGRAL_CONSTANT_PID) / 8;
    if(integral_term > (motor_list[M_index].max_pwm_voltage << PID_SHIFT_RIGHT)) {
    	integral_term = motor_list[M_index].max_pwm_voltage << PID_SHIFT_RIGHT;
    }
    else if(integral_term < -(motor_list[M_index].max_pwm_voltage << PID_SHIFT_RIGHT)) {
    	integral_term = -(motor_list[M_index].max_pwm_voltage << PID_SHIFT_RIGHT);
    }
    ...
    // derivative_term = low_pass_filtered_error_change * DERIVATIVE_CONSTANT_PID;
		...
		// derivative_term = error_change * PWM_PERIOD * DERIVATIVE_CONSTANT_PID;
		derivative_term = (error_change * DERIVATIVE_CONSTANT_PID) >> 16;

    previous_error = error;
    output_value = (integral_term + proportional_term + derivative_term) >> PID_SHIFT_RIGHT;
		// output_value = (proportional_term + derivative_term) >> PID_SHIFT_RIGHT;
		// output_value = (proportional_term + integral_term) >> PID_SHIFT_RIGHT;
		// output_value = (proportional_term) >> PID_SHIFT_RIGHT;
...
void motor_movement_calculations(void)
{
    ...
    if(motor_list[M_index].go_to_closed_loop_mode_active) {
        go_to_closed_loop_mode_logic();
    }
    else if(motor_list[M_index].homing_active) {
        handle_homing_logic();
    }
    else if(motor_list[M_index].calibration_active) {
        handle_calibration_logic();
    }

    moving = handle_queued_movements();

	if( (((int32_t *)&motor_list[M_index].current_position_i64)[1] > motor_list[M_index].position_upper_safety_limit) || (((int32_t *)&motor_list[M_index].current_position_i64)[1] < motor_list[M_index].position_lower_safety_limit) ) {
		fatal_error(25); // "safety limit exceeded" (all error text is defined in error_text.c)
	}

	if(motor_list[M_index].motor_control_mode == OPEN_LOOP_POSITION_CONTROL) {
		motor_list[M_index].commutation_position = ((int32_t *)(motor_list[M_index].current_position_i64 >> 32) + motor_list[M_index].commutation_position_offset);
		if(moving) {
			motor_list[M_index].motor_pwm_voltage = motor_list[M_index].max_pwm_voltage;
		}
		else {
			motor_list[M_index].motor_pwm_voltage = OPEN_LOOP_STATIC_MOTOR_PWM_VOLTAGE;
		}
	}
	else {
		motor_list[M_index].commutation_position = motor_list[M_index].hall_position + motor_list[M_index].commutation_position_offset;
		if(motor_list[M_index].motor_control_mode == CLOSED_LOOP_POSITION_CONTROL) {
			motor_list[M_index].motor_pwm_voltage = PID_controller(((int32_t *)&motor_list[M_index].current_position_i64)[1] - motor_list[M_index].hall_position);
			// int32_t back_emf_voltage = (motor_list[M_index].velocity * VOLTS_PER_ROTATIONAL_VELOCITY) >> 8;
			int32_t back_emf_voltage = 0;
			int32_t motor_max_allowed_pwm_voltage = back_emf_voltage + motor_list[M_index].max_pwm_voltage;
			int32_t motor_min_allowed_pwm_voltage = back_emf_voltage - motor_list[M_index].max_pwm_voltage;
			// int32_t motor_max_allowed_pwm_voltage = motor_list[M_index].max_pwm_voltage;
			// int32_t motor_min_allowed_pwm_voltage = motor_list[M_index].max_pwm_voltage;
			if(motor_list[M_index].motor_pwm_voltage > motor_max_allowed_pwm_voltage) {
				motor_list[M_index].motor_pwm_voltage = motor_max_allowed_pwm_voltage;
			}
			if(motor_list[M_index].motor_pwm_voltage < motor_min_allowed_pwm_voltage) {
				motor_list[M_index].motor_pwm_voltage = motor_min_allowed_pwm_voltage;
			}
			if(motor_list[M_index].motor_pwm_voltage >= 0) {
				motor_list[M_index].commutation_position += HALL_TO_POSITION_90_DEGREE_OFFSET;
			}
			else {
				motor_list[M_index].commutation_position -= HALL_TO_POSITION_90_DEGREE_OFFSET;
				motor_list[M_index].motor_pwm_voltage = -motor_list[M_index].motor_pwm_voltage;
			}
		}
		else {
			if(motor_list[M_index].desired_motor_pwm_voltage >= 0) {
				motor_list[M_index].commutation_position += HALL_TO_POSITION_90_DEGREE_OFFSET;
				motor_list[M_index].motor_pwm_voltage = motor_list[M_index].desired_motor_pwm_voltage;
			}
			else {
				motor_list[M_index].commutation_position -= HALL_TO_POSITION_90_DEGREE_OFFSET;
				motor_list[M_index].motor_pwm_voltage = -motor_list[M_index].desired_motor_pwm_voltage;
...
//    transmit(buf, strlen(buf));
    ...
    commutation_step = (motor_list[M_index].commutation_position >> N_COMMUTATION_SUB_STEPS_SHIFT_RIGHT) % N_COMMUTATION_STEPS;
    commutation_sub_step = (motor_list[M_index].commutation_position & 0xff) >> (8 - N_COMMUTATION_SUB_STEPS_SHIFT_RIGHT);
...
//    for(delay = 0; delay < 100000; delay++);
    ...
    phase1 >>= 8;
    phase1 *= (uint32_t)motor_list[M_index].motor_pwm_voltage;
    phase1 >>= 16;
    phase1 += 13;

    phase2 >>= 8;
    phase2 *= (uint32_t)motor_list[M_index].motor_pwm_voltage;
    phase2 >>= 16;
    phase2 += 13;

    phase3 >>= 8;
    phase3 *= (uint32_t)motor_list[M_index].motor_pwm_voltage;
    phase3 >>= 16;
    phase3 += 13;
...
#define MAX_HALL_POSITION_DELTA_FATAL_ERROR_THRESHOLD 100000
void TIM1_BRK_UP_TRG_COM_IRQHandler(void)
{
	...
	motor_list[M_index].hall_position_delta = motor_list[M_index].hall_position - motor_list[M_index].previous_hall_position;
	motor_list[M_index].previous_hall_position = motor_list[M_index].hall_position;

	if(motor_list[M_index].hall_position_delta > max_hall_position_delta) {
		max_hall_position_delta = motor_list[M_index].hall_position_delta;
	}
	if(motor_list[M_index].hall_position_delta < min_hall_position_delta) {
		min_hall_position_delta = motor_list[M_index].hall_position_delta;
	}
	average_hall_position_delta += motor_list[M_index].hall_position_delta;
	...
	if(fast_capture_data_active != 0) {
		...
		fast_capture_data[fast_capture_data_index].hall_position_16bit = (uint16_t)motor_list[M_index].hall_position;
		fast_capture_data_index++;
		if(fast_capture_data_index >= fast_capture_data_size) {
	...
	if((motor_list[M_index].hall_position_delta < -MAX_HALL_POSITION_DELTA_FATAL_ERROR_THRESHOLD) || (motor_list[M_index].hall_position_delta > MAX_HALL_POSITION_DELTA_FATAL_ERROR_THRESHOLD)) {
		disable_mosfets();
	...
	// if((((int32_t *)&motor_list[M_index].current_position_i64)[1] > POSITION_OUT_OF_RANGE_FATAL_ERROR_THRESHOLD) || (((int32_t *)&motor_list[M_index].current_position_i64)[1] < -POSITION_OUT_OF_RANGE_FATAL_ERROR_THRESHOLD)) {
	...
	// if((motor_list[M_index].hall_position > POSITION_OUT_OF_RANGE_FATAL_ERROR_THRESHOLD) || (motor_list[M_index].hall_position < -POSITION_OUT_OF_RANGE_FATAL_ERROR_THRESHOLD)) {
...
void increase_motor_pwm_voltage(void)
{
  ...
  TIM1->DIER &= ~TIM_DIER_UIE; // disable the update interrupt during this operation
	motor_list[M_index].desired_motor_pwm_voltage++;
  TIM1->DIER |= TIM_DIER_UIE; // enable the update interrupt
	sprintf(buf, "desired_motor_pwm_voltage: %d\n", (int)motor_list[M_index].desired_motor_pwm_voltage);
	transmit(buf, strlen(buf));
}
...
{
	motor_list[M_index].motor_control_mode = new_motor_control_mode;
}

uint8_t get_motor_control_mode(void)
{
	return motor_list[M_index].motor_control_mode;
}
...
//	commutation_position_offset = commutation_position_offset - ((int32_t *)&current_position_i64)[1];
	  motor_list[M_index].commutation_position_offset = motor_list[M_index].commutation_position_offset + get_hall_position();
    zero_hall_position();
	  motor_list[M_index].current_position_i64 = 0;
	  motor_list[M_index].current_velocity_i64 = 0;
    TIM1->DIER |= TIM_DIER_UIE; // enable the update interrupt
...
{
    TIM1->DIER &= ~TIM_DIER_UIE; // disable the update interrupt during this operation
		motor_list[M_index].max_velocity = new_max_velocity;
		motor_list[M_index].max_velocity <<= VELOCITY_SHIFT_LEFT;
		if(motor_list[M_index].max_velocity > MAX_VELOCITY) {
			motor_list[M_index].max_velocity = MAX_VELOCITY;
		}
    TIM1->DIER |= TIM_DIER_UIE; // enable the update interrupt
}
...
{
	return motor_list[M_index].max_velocity;
}

void set_max_acceleration(uint32_t new_max_acceleration)
{
    TIM1->DIER &= ~TIM_DIER_UIE; // disable the update interrupt during this operation
		motor_list[M_index].max_acceleration = new_max_acceleration;
		motor_list[M_index].max_acceleration <<= ACCELERATION_SHIFT_LEFT;
		if(motor_list[M_index].max_acceleration > MAX_ACCELERATION) {
			motor_list[M_index].max_acceleration = MAX_ACCELERATION;
		}
    TIM1->DIER |= TIM_DIER_UIE; // enable the update interrupt
}
...
{
	return motor_list[M_index].max_acceleration;
}

int32_t get_current_position(void)
{
	return ((int32_t *)&motor_list[M_index].current_position_i64)[1];
}
...
int32_t get_actual_motor_position(void)
{
    ...
    TIM1->DIER &= ~TIM_DIER_UIE; // disable the update interrupt during this operation
		if(motor_list[M_index].motor_control_mode == CLOSED_LOOP_POSITION_CONTROL) {
			ret = motor_list[M_index].hall_position;
		}
		else {
			ret = ((int32_t *)&motor_list[M_index].current_position_i64)[1];
		}
    TIM1->DIER |= TIM_DIER_UIE; // enable the update interrupt
    return ret;
}
...
uint8_t get_motor_status_flags(void)
{
	...
	if(motor_list[M_index].calibration_active) {
		motor_status_flags |= (1 << STATUS_CALIBRATING_FLAG_BIT);
	}
	if(motor_list[M_index].homing_active) {
...
{
	motor_list[M_index].motor_current_baseline = get_motor_current();
	if((motor_list[M_index].motor_current_baseline < MIN_MOTOR_CURRENT_BASELINE) || (motor_list[M_index].motor_current_baseline > MAX_MOTOR_CURRENT_BASELINE)) {
		fatal_error(22);
	}
}

// void set_PID_control_p_value(uint32_t p_value)
// {
// 	motor_list[M_index].p_value = p_value;
// 	global_settings.p_value = p_value;
// }
//
// void set_PID_control_i_value(uint32_t i_value)
// {
// 	motor_list[M_index].i_value = i_value;
// 	global_settings.i_value = i_value;
// }
//
// void set_PID_control_d_value(uint32_t d_value)
// {
// 	motor_list[M_index].d_value = d_value;
// 	global_settings.d_value = d_value;
// }
...
void set_max_motor_current(uint16_t new_max_pwm_voltage, uint16_t new_max_regen_pwm_voltage)
{
	...
	for(int i = 0; i < NUMBER_OF_MOTORS; i++) {
		motor_list[i].max_pwm_voltage = new_max_pwm_voltage;
		...
		set_analog_watchdog_limits(motor_list[i].motor_current_baseline - analog_watchdog_lower_limit, motor_list[i].motor_current_baseline + analog_watchdog_upper_limit);
	}
}

void set_movement_limits(int32_t lower_limit, int32_t upper_limit)
{
	motor_list[M_index].position_lower_safety_limit = lower_limit;
	motor_list[M_index].position_upper_safety_limit = upper_limit;
}
