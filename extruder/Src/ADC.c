...
//                 (0xf << ADC_CHSELR_SQ8_Pos);
    ...
    ADC1->CHSELR = (0  << ADC_CHSELR_SQ1_Pos) |  // motor 1 current        (index 0)
                   (5  << ADC_CHSELR_SQ2_Pos) |  // hall 1                 (index 1)
                   (4  << ADC_CHSELR_SQ3_Pos) |  // hall 2                 (index 2)
                   (0  << ADC_CHSELR_SQ4_Pos) |  // motor 1 current        (index 3)
                   (6  << ADC_CHSELR_SQ5_Pos) |  // hall 3                 (index 4)
                   (7  << ADC_CHSELR_SQ6_Pos) |  // hall 4                 (index 5)
                   (0  << ADC_CHSELR_SQ7_Pos) |  // motor 1 current        (index 6)
                   (10 << ADC_CHSELR_SQ8_Pos);   // temperature sensor     (index 7)
...
{
    uint16_t a = ADC_buffer[HALL4_ADC_CYCLE_INDEX + 0] + ADC_buffer[HALL4_ADC_CYCLE_INDEX + 8] + ADC_buffer[HALL4_ADC_CYCLE_INDEX + 16] + ADC_buffer[HALL4_ADC_CYCLE_INDEX + 24];
/*  if(a < 8500 - 2000) {
        fatal_error(ERROR_DEBUG1); // All error messages are defined in error_text.h, which is an autogenerated file based on error_codes.json in the servomotor Python module (<repo root>/python_programs/servomotor/error_codes.json)
    }
    if(a > 8500 + 2000) {
...
{
    uint16_t a = ADC_buffer[TEMPERATURE_ADC_CYCLE_INDEX + 0] + ADC_buffer[TEMPERATURE_ADC_CYCLE_INDEX + 8] + ADC_buffer[TEMPERATURE_ADC_CYCLE_INDEX + 16] + ADC_buffer[TEMPERATURE_ADC_CYCLE_INDEX + 24];
    if(a > MAX_TEMPERATURE_SENSOR_ADC_VALUE) {
        a = MAX_TEMPERATURE_SENSOR_ADC_VALUE;
    }
    return a;
}
