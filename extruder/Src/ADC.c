...
//				   (0xf << ADC_CHSELR_SQ8_Pos);
    ADC1->CHSELR = (10 << ADC_CHSELR_SQ1_Pos) |    // select the channels to be converted, 8 channels total
...
{
    return ADC_buffer[7] + ADC_buffer[7 + 8] + ADC_buffer[7 + 16] + ADC_buffer[7 + 24];
}

uint16_t get_temperature_sensor_voltage(void)
{
    return ADC_buffer[0] + ADC_buffer[0 + 8] + ADC_buffer[0 + 16] + ADC_buffer[0 + 24];
}
