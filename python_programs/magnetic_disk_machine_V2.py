...
N_DISKS = 15 # 18 is maximum in the current setup
N_POLES = 50

MOSFET_REGEN_CURRENT = 0
HIGHEST_MOSFET_CURRENT = 390

# you can characterize the needed MOSFET current using the tool called characterize_homing.py
UP_DOWN_MOSFET_CURRENT = 50
SLIDER_MOSFET_CURRENT = 50
DISK_SPIN_MOSFET_CURRENT = 50
DISK_TRAY_MOSFET_CURRENT = 50
...
motor255 = servomotor.M3(255)
motor = servomotor.M3(255, time_unit="timesteps", position_unit="encoder_counts", velocity_unit="counts_per_timestep", acceleration_unit="counts_per_timestep_squared", current_unit="internal_current_units", voltage_unit="volts", temperature_unit="celsius")
motor_disk_spin = servomotor.M3(DISK_SPIN_ALIAS, time_unit="timesteps", position_unit="encoder_counts", velocity_unit="counts_per_timestep", acceleration_unit="counts_per_timestep_squared", current_unit="internal_current_units", voltage_unit="volts", temperature_unit="celsius")
motor_slider = servomotor.M3(SLIDER_ALIAS, time_unit="timesteps", position_unit="encoder_counts", velocity_unit="counts_per_timestep", acceleration_unit="counts_per_timestep_squared", current_unit="internal_current_units", voltage_unit="volts", temperature_unit="celsius")
motor_disk_tray = servomotor.M3(DISK_TRAY_ALIAS, time_unit="timesteps", position_unit="encoder_counts", velocity_unit="counts_per_timestep", acceleration_unit="counts_per_timestep_squared", current_unit="internal_current_units", voltage_unit="volts", temperature_unit="celsius")
motor_up_down = servomotor.M3(UP_DOWN_ALIAS, time_unit="timesteps", position_unit="encoder_counts", velocity_unit="counts_per_timestep", acceleration_unit="counts_per_timestep_squared", current_unit="internal_current_units", voltage_unit="volts", temperature_unit="celsius")
...
UP_DOWN_POSITION_FOR_MAGNET_PLACEMENT = 453
...
POLE_PLACE_ANGLE = EXPECTED_SLIDER_RANGE / 2 * 0.90 # a certain fraction of the way to the end stop
...
def do_homing_and_get_position_in_degrees(alias, max_degrees, max_time_s, homing_n_times = 1, relief_degrees = None, homing_tolerance_degrees = None, start_position = None,
    ...
    for i in range(homing_n_times):
        ...
        parsed_response = motor.homing(rotation_motor_units, movement_time_device_units)

        # wait for the motor to finish homing
        while True:
            parsed_response = motor.get_status()
        ...
        parsed_response = motor.get_hall_sensor_position()
        position_motor_units = parsed_response
        ...
        if relief_degrees is not None and i < homing_n_times - 1:
            ...
            parsed_response = motor.go_to_position(position_motor_units + rotation_motor_units, movement_time_device_units)
            time.sleep(0.5)
    ...
    if zero_after_homing:
        ...
        parsed_response = motor.zero_position()
    if start_position is not None:
        ...
        parsed_response = motor.go_to_position(rotation_motor_units, movement_time_device_units)
        time.sleep(move_time_s * 1.1)
        # read the position and see if we are where we expect to be. if not then we exit
        parsed_response = motor.get_position()
        position_motor_units = parsed_response
...
def disable_mosfets_and_read_positions():
    ...
    parsed_response = motor_disk_spin.disable_mosfets()
    parsed_response = motor_slider.disable_mosfets()
    parsed_response = motor_up_down.disable_mosfets()
    parsed_response = motor_disk_tray.disable_mosfets()
    while 1:
        # get the positions of all three motors
        parsed_response = motor_disk_spin.get_position()
        disk_spin_position = parsed_response
        parsed_response = motor_slider.get_position()
        slider_position = parsed_response
        parsed_response = motor_up_down.get_position()
        up_down_position = parsed_response
        parsed_response = motor_disk_tray.get_position()
        disk_tray_position = parsed_response
...
#        cumulative_disk_angle += 360.0 / N_POLES / 50 * 0.25 # small fudge factor to add some tolerance spacing between magnets
        ...
        parsed_response = motor_disk_spin.go_to_position(cumulative_position_motor_units, movement_time_device_units)
...
#            parsed_response = motor_slider.go_to_position(ONE_DEGREE_MICROSTEPS * pole_place_angle, 31250 * move_time_s], verbose=VERBOSE)
#            time.sleep(move_time_s * 1.1)
        ...
        while 1:
            ...
            parsed_response = motor_slider.go_to_position(ONE_DEGREE_MICROSTEPS * pole_place_angle, 31250 * move_time_s)
            time.sleep(move_time_s * 1.5)
            ...
            if abs(position_error) > DISK_PLACE_SLIDER_POSITION_TOLERANCE:
                ...
                # now, move the slider motor to place that pole onto the disk
                parsed_response = motor_slider.go_to_position(0, 31250 * move_time_s)
                time.sleep(move_time_s * 1.5)
                parsed_response = motor_slider.disable_mosfets()
                ...
                while 1:
                    ...
                    if user_input == "q":
                        ...
                        parsed_response = motor_slider.enable_mosfets()
                        break
            else:
                break
        ...
        if i == 1:
            ...
            parsed_response = motor_slider.go_to_position(ONE_DEGREE_MICROSTEPS * pole_place_angle * 0.9, 31250 * move_time_s)
            time.sleep(move_time_s * 1.1)
            user_input = get_user_input("Press a key to continue")
            disk_angle = cumulative_disk_angle + 360.0 / N_POLES * 1.5
            parsed_response = motor_disk_spin.go_to_position(ONE_DEGREE_MICROSTEPS * disk_angle, 31250 * move_time_s)
            time.sleep(move_time_s * 1.1)
            user_input = get_user_input("Press a key to continue")
            parsed_response = motor_slider.go_to_position(ONE_DEGREE_MICROSTEPS * -EXPECTED_SLIDER_RANGE * 0.5, 31250 * move_time_s)
            time.sleep(move_time_s * 1.1)
            user_input = get_user_input("Press a key to continue")
        ...
        move_time_s = 0.3
        rotation = 0
        parsed_response = motor_slider.go_to_position(ONE_DEGREE_MICROSTEPS * rotation, 31250 * move_time_s)
        time.sleep(move_time_s * 1.1)
...
for alias in [DISK_SPIN_ALIAS, SLIDER_ALIAS, UP_DOWN_ALIAS]:
    ...
    for i in range(N_PINGS_TO_TEST_COMMUNICATION):
        ...
        motor.use_this_alias_or_unique_id(ord(alias))
        parsed_response = motor.ping(random_10_bytes, verbose=VERBOSE)
        if parsed_response != random_10_bytes:
...
motor_disk_spin.set_pid_constants(PID_P, PID_I, PID_D)
motor_slider.set_pid_constants(PID_P, PID_I, PID_D)
motor_up_down.set_pid_constants(PID_P, PID_I, PID_D)
motor_disk_tray.set_pid_constants(PID_P, PID_I, PID_D)

# enable MOSFETs in all three motors
motor_disk_spin.enable_mosfets()
motor_slider.enable_mosfets()
motor_up_down.enable_mosfets()
motor_disk_tray.enable_mosfets()

# go to close loop position control mode in all three motors
motor_disk_spin.go_to_closed_loop()
motor_slider.go_to_closed_loop()
motor_up_down.go_to_closed_loop()
motor_disk_tray.go_to_closed_loop()

# set the MOSFET current
motor_disk_spin.set_maximum_motor_current(DISK_SPIN_MOSFET_CURRENT, MOSFET_REGEN_CURRENT)
motor_slider.set_maximum_motor_current(SLIDER_MOSFET_CURRENT, MOSFET_REGEN_CURRENT)
motor_up_down.set_maximum_motor_current(UP_DOWN_MOSFET_CURRENT, MOSFET_REGEN_CURRENT)
motor_disk_tray.set_maximum_motor_current(DISK_TRAY_MOSFET_CURRENT, MOSFET_REGEN_CURRENT)
...
for disk_number in range(N_DISKS):
    ...
    movement_time_device_units = int(31250 * move_time_s)
    print("*********************************** rotation_motor_units = ", rotation_motor_units)
    parsed_response = motor_disk_tray.go_to_position(rotation_motor_units, movement_time_device_units)
    time.sleep(move_time_s * 1.1)
    ...
    parsed_response = motor_up_down.go_to_position(rotation_motor_units, movement_time_device_units)
    rotation_motor_units = int(ONE_DEGREE_MICROSTEPS * 360 * DISK_PICKUP_SPIN_ROTATIONS)
    parsed_response = motor_disk_spin.go_to_position(rotation_motor_units, movement_time_device_units)
    time.sleep(move_time_s * 1.1)
    ...
    parsed_response = motor_up_down.go_to_position(rotation_motor_units, movement_time_device_units)
    parsed_response = motor_disk_spin.go_to_position(0, movement_time_device_units)
    time.sleep(move_time_s * 1.1)
    ...
    if disk_number == 0: # we will use the first disk for homing of the slider. if magents are loaded then one or two magnets will be pushed onto the disk. since they won't be positioned right, we will not full assemble the disk
        ...
        parsed_response = motor_slider.go_to_position(rotation_motor_units, movement_time_device_units)
        time.sleep(move_time_s * 1.1)
        parsed_response = motor_slider.zero_position()
    else:
