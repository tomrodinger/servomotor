...
def test_homing():
    ...
    try:
        ...
        motorX.set_position_unit("shaft_rotations")  # Set back to shaft_rotations for homing
        ...
        print("Homing with distance of +6553600 counts...")
        start_time = time.time()
        motorX.set_position_unit("encoder_counts")  # Already in encoder counts
        start_position = motorX.get_position()
        motorX.homing(maxDistance=6553600, maxDuration=2)  # Home with +6553600 counts distance, 2 second limit
        wait_for_moves_to_complete(motorX)
        elapsed_time = time.time() - start_time
        print(f"Positive direction homing completed in {elapsed_time:.3f} seconds")
        verify_homing_movement(motorX, start_position, 1, 6553600)  # 6,553,600 counts = 2 rotations

        print("\nHoming with distance of -6553600 counts...")
        start_time = time.time()
